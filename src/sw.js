...
self.addEventListener('install', (event) => {
  ...
  event.waitUntil(
    caches.open(CACHE_NAME)
      .then((cache) => {
        logger.log('[ServiceWorker] Pre-caching core structural assets');
        return cache.addAll(CORE_ASSETS);
      })
      .catch((error) => {
        logger.warn('[ServiceWorker] Pre-caching encountered an error (some paths may not exist yet):', error);
      })
  );
});
      ...
      return Promise.all(
        cacheNames.map((cacheName) => {
          if (cacheName !== CACHE_NAME) {
            logger.log('[ServiceWorker] Removing old cache', cacheName);
  ...
  event.respondWith(
    caches.match(event.request).then((cachedResponse) => {
      ...
      const fetchPromise = fetch(event.request).then((networkResponse) => {
        ...
        logger.warn('[ServiceWorker] Network request failed, relying on cache for:', event.request.url);
      });
